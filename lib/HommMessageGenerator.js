class HommMessageGenerator {
  ...
  findMessageWindowSize() {
    ...
    for(var i=1;i<breakpoints.length;i++) {
      ...
      if(this.textLength < breakpoint.at) {
  ...
  drawMessageWindow() {
    ...
    for(var i=1;i<this.message_size[0]-1;i++) {
      ...
      var y_to_set = this.getCanvasHeight() - this.horizontal_border_height - this.getShadowHeight();
      // upper
      this.context.drawImage(
        this.sprite,
        this.border_size * 2,
    ...
    for(var i=1;i<this.message_size[1]-1;i++) {
      var right_x_to_set = this.getCanvasWidth() - this.vertical_border_width - this.getShadowWidth();
